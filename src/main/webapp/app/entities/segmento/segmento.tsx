...
export const Segmento = (props: ISegmentoProps) => {
  ...
  const { segmentoList, match, loading, totalItems } = props;
  return (
    <div>
                ...
                <tr key={`entity-${i}`}>
                  ...
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${segmento.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${segmento.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
      ...
      {props.totalItems ? (
        <div className={segmentoList && segmentoList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ segmento }: IRootState) => ({
  ...
  totalItems: segmento.totalItems,
});
